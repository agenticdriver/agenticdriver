...
export function registerProviderPanel(
  ...
  class Panel extends HTMLElement implements ProviderPanelElement {
    ...
    private async setupRequest(request: ProviderSetupRequest) {
      ...
      try {
        ...
        if (
          ...
          this.state.setup = {
            version: 1,
            attempts: [
              ...(this.state.setup?.attempts ?? []).filter(
                (a) => !changed.has(a.id),
              ),
              ...result.attempts,
            ],
          ...
          if (request.action === "start") {
            ...
            this.apiKey = "";
            this.draft = structuredClone(
              this.state.management?.providers.find(
    ...
    private armSetupPoll() {
      clearTimeout(this.setupTimer);
      if (
        !this.isConnected ||
        !this.state?.setup?.attempts.some((a) =>
          ["starting", "waiting", "verifying", "ready"].includes(a.phase),
        )
      )
        return;
    ...
    private async removeProvider() {
      ...
      const provider = management?.providers.find(
        (p) => p.id === this.selected,
      );
      if (
        !this.removalPending ||
    ...
    private async onClick(event: Event) {
      ...
      if (action === "remove-provider") {
        ...
        this.render();
        this.root
          .querySelector<HTMLElement>('[data-action="cancel-remove"]')
          ?.focus();
        return;
      }
      if (action === "cancel-remove") {
        ...
        this.render();
        this.root
          .querySelector<HTMLElement>('[data-action="remove-provider"]')
          ?.focus();
        return;
      }
      if (action === "confirm-remove") return this.removeProvider();
      if (
        (action === "setup-accept" || action === "setup-cancel") &&
        button.dataset.id
      ) {
    ...
    private definitions(): ProviderDefinition[] {
      ...
      return (management?.providerDefinitions ?? [])
        ...
        .map((p) => ({
          ...p,
          methods: p.methods.filter(
            (m) =>
              m.interaction !== "device-code" ||
    ...
    private setupProvider(): string {
      ...
      let advanced =
        ...
        this.field(
          ...
          "accountId",
          p.accountId,
          method?.interaction === "device-code"
    ...
    private removalSection() {
      const provider = this.state?.management?.providers.find(
        (p) => p.id === this.selected,
      );
      if (
        this.adding ||
