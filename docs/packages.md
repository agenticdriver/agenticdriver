...
`/providers`, `/usagestat`, `/context`, and `/retrieval`. Each supplies its own
TypeScript declarations. This contract checks a Node.js consumer; browser
bundling, other language package releases and publishing automation remain
separate AD-042 work. The existing `npm run test:install` continues to exercise
the installed host CLI lifecycle.
