...
// npm.cmd is not an executable file on Windows. Invoke npm's JS entry without a shell.
const npmEntry = process.env.npm_execpath;
assert.ok(npmEntry, "Run this check through npm run test:install");
const installOnly = process.argv.includes("--package-only");
...
try {
  ...
  await run(
    process.execPath,
    [npmEntry, "pack", "--pack-destination", directory, "--silent"],
    {
      cwd: root,
      timeout: 120_000,
      maxBuffer: 2_000_000,
  ...
  `,
    ...
    assert.equal(
      JSON.stringify(records).includes(env.AD_INSTALL_API_KEY),
      false,
    );
    ...
    process.stdout.write(
      "Installed package, programmatic host and browser/client examples passed; service CLI signal lifecycle is outside this matrix row.\n",
    );
  }
} finally {
