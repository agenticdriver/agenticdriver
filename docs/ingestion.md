...
There is no default execution deadline or inactivity timeout. An explicit positive `idleTimeoutMs` resets on real extraction, chunking, embedding, authorization or store progress. A host inactivity policy can impose a smaller limit. No heartbeat resets that clock, and zero/omitted values do not disable a configured host limit. TypeScript/Go also accept cancellation signals/contexts. Python supports both synchronous and native asyncio ingestion; cancelling its async task closes the response. Rust supports async future cancellation and an optional blocking transport. A custom extractor must honor the execution context's signal and report actual progress.
