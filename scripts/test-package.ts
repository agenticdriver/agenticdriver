...
assert.ok(npmEntry, "Run this check through npm run test:package");
...
async function command(binary: string, argv: string[], cwd: string) {
  try {
    return await run(
      binary === "npm" ? process.execPath : binary,
      binary === "npm" ? [npmEntry!, ...argv] : argv,
      {
        cwd,
        env,
        timeout: 120_000,
...
try {
    ...
    JSON.stringify(
      {
        ...
        type: "module",
        dependencies: {
          "@agenticdriver/sdk": `file:../pack-0/${archive.filename}`,
        },
        devDependencies: {
  ...
  assert.equal(manifest.name, "@agenticdriver/sdk");
  ...
  // TypeScript emits forward slashes on Windows; compare canonical paths for
  // both compiler inputs and advertised declarations on every platform.
